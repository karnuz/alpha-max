...
methodList={'AlphaMax','AlphaMax_N','Elkan_Noto','GMM_T','PDF_Ratio','CDF_Based','AlphaMax_B'};
...
methodFcn={@alphamax,@alphamaxN,@en,@gmm_t,@pdf_ratio,@cdf_based,@alphamaxB};
...
function [est,out]=alphamaxB(x,x1,opts)
...
[alphas,fs,out.compute_llCurve]=compute_llCurve_B1(x,x1,opts);
% Ensures that the LL curve is non-increasing
fs=llCurve_correction(alphas,fs);

out.alphas=alphas;
out.fs=fs;
out.betas = out.compute_llCurve.betas;
out.zetas = out.compute_llCurve.zetas;
